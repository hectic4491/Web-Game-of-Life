...
class Simulation ():
    ...
    def _generate_grid(self):
        ...
        for j in range(self.height):
            for i in range(self.width):
                ...
                cell.neighbors[2] = (grid[j-1, (i+1) % self.width])

                cell.neighbors[3] = (grid[j, i-1])
                cell.neighbors[4] = (grid[j, (i+1) % self.width])

                cell.neighbors[5] = (grid[(j+1) % self.height, i-1])
                cell.neighbors[6] = (grid[(j+1) % self.height, i])
                cell.neighbors[7] = (grid[(j+1) % self.height,
                                          (i+1) % self.width])
    ...
    def _write_gen_data(self, step):
        ...
        gen_data = {"alive": None,"population": None,"generation": None}
        alive = np.empty(self.grid.size, dtype=object)

        n = 0
        for j in range(self.height):
            for i in range(self.width):
                if self.grid[j, i].alive:
                    alive[n] = [j, i]
                    n += 1
                # n will also tell us the population count of that gen.

        gen_data["alive"] = alive.tolist()
        gen_data["population"] = n
        gen_data["generation"] = step

        ### TODO ###:
        # create the alive and dead lists to aid
...
if __name__ == '__main__':
    ...
    print(myData.render_data[99]["generation"])
